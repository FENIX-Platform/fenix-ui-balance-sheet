define(["jquery", "fx-balance-sheet/formatter/DatatypesFormatter",
    "fx-balance-sheet/flagTranslator/controller/FlagController",
    "text!modalView/fx-balance-sheet/modalForm.html", "select2",
    ...
    CellEditor.prototype.init = function (Configurator, cell, dsd) {
        ...
        for (var i = 0; i < leftKeyColumnsIndexes.length; i++) {
            ...
            $('#fx-ui-balance-sheet_form').append("<div class ='row'>" +
        ...
        for (var i = 0; i < upKeyColumnsIndexes.length; i++) {
            ...
            $('#fx-ui-balance-sheet_').append("<div class ='row'>" +
        ...
        $("#fx-ui-balance-sheet_dialogForm").modal({ backdrop: 'static',
            keyboard: false});


        $("#fx-ui-balance-sheet_dialogForm").draggable({
            handle: ".modal-header"
        });



        $('#fx-ui-balance-sheet_resetButton').on('click', function (event) {
    ...
    CellEditor.prototype.restorePreviousValues = function () {
        ...
        for (var key in mapPreviousValues) {
            if (key == 'getUnique') {
                break;
            }
    ...
    CellEditor.prototype.destroyIfExist = function () {
        ...
        $('#fx-balance-sheet_specialForm').modal('hide');

        var g = document.getElementById("fx-ui-balance-sheet_specialForm");

        if (g && g !== null) {
            g.remove()
        }

        var k =  $('#fx-ui-balance-sheet_closeModal');


        if(k) {
            $('#fx-ui-balance-sheet_closeModal').click();
        }

        $('#fx-ui-balance-sheet_dialogForm').modal('hide');


        var f =  $('#closeModalFormTotal');
        if(f) {
            $('#fx-ui-balance-sheet_closeModalFormTotal').click();
        }

        var f = document.getElementById("fx-ui-balance-sheet_dialogForm");
